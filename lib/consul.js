...
exports.Consul.prototype.register = function (next) {
    ...
    return Items.parallel(server.connections, function (connection, nextItem) {
        ...
        var initialState = self.service.check.startHealthy ? 'passing' : undefined;
        ...
        if (self.service.check.ttl) {
            serviceInfo.checks.push({
                ttl: self.service.check.ttl,
                status: initialState
            });
        }
...
exports.Consul.prototype.checkin = function (ok, note, callback) {
    ...
    var method = ok === true ? 'pass' : ok === false ? 'fail' : 'warn';
    var checkInstance = self.service.check.path ? ':2' : '';

    server.log(['attache', 'consul', 'checkin'], 'Performing check-in with state', ok);

    return Items.parallel(server.connections, function (connection, nextItem) {

        self.api.agent.check[method]({
            id: 'service:' + self.connectionId(connection) + checkInstance,
            note: note || undefined
        }, function (err) {
